...
| 职业测评 `/assessment` | 部分实现 | 通用/行业岗位测评、DeepSeek 动态题库、8 题缓存、本地题库回退，以及由主方向、证据、差距和 7 天任务组成的职业决策报告。 | 不是经过心理测量验证的正式量表；报告中的岗位方向是待验证假设，不是录用预测或真实职位；历史数据权限保护不足。 |
...
5. 用户点击“完成并生成分析”后，页面只发起一次 DeepSeek 请求，生成 `CareerProfile`：主方向、最多两个备选方向、带来源的证据、待补齐差距与最多三项 7 天任务。没有简历时仍可生成，并在模型失败时回落到本地基础方案。
6. 结果页只显示职业决策报告，不显示总分、雷达图、人格标签或跨测评趋势。用户可修改主目标岗位；报告明确该方向只是待验证假设。
7. 本次报告、最多 12000 字的简历文本和 `CareerProfile` 写入 `WorkflowContext`；用户确认主方向时，系统生成一个标记为“非真实职位”的 `selectedJob` 作为岗位验证、简历与面试的共同上下文。若登录，额外尝试写入测评记录。
...
1. 拉取行业和岗位 taxonomy。
2. 接受简历文本与筛选条件。
3. 将简历、`CareerProfile`、行业/岗位条件交给 DeepSeek 生成最多六个岗位方向；主目标方向优先展示。
