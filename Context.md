...
| 邮箱注册与登录 | 部分实现 | 邮箱+密码注册、邮箱+密码登录、浏览器本地会话恢复；从测评或面试触发登录后返回原任务。 | 登录与注册仍是两个路由；无邮箱验证码、找回密码、限流或刷新令牌。 |
| 职业测评 `/assessment` | 部分实现 | 通用/行业岗位测评、DeepSeek 动态题库、本地题库回退，以及由主方向、证据、差距和 7 天任务组成的职业决策报告；最近 10 版报告写入云端工作流。 | 不是经过心理测量验证的正式量表；报告中的岗位方向是待验证假设，不是录用预测或真实职位。 |
| 岗位匹配 `/jobs` | 部分实现 | 用户填写行业、目标岗位、城市、经验、薪资底线和工作方式后，主动授权并生成最多 3 个待验证岗位方向；结果写入云端工作流。 | 不是实时招聘市场数据；没有职位爬取、投递、收藏或职位真实性校验。 |
| 简历优化 `/resume` | 部分实现 | PDF/DOCX 文本提取、粘贴文本、真实 JD 对比、AI JSON 草稿、事实校验、可复制正文、中文/英文模板 Word 下载、最近 10 条简历记录。 | 岗位方向不再冒充真实 JD；AI 失败、占位符或新增原简历不存在的数字时不生成/保存/导出；原文件二进制未上云，历史旧草稿只允许预览。 |
| AI 面试 `/interview` | 部分实现 | 自由岗位/JD/职责上下文、四类面试、开始前锁定 AI 或本地题库、主问题+追问、文字/语音回答、按需设备检测、普通话女声 TTS、TXT 下载和最近 10 次云端报告。 | AI 面试官是文字与语音体验，不是视频数字人；结果是训练记录而非能力评分；浏览器女声音色取决于操作系统可用语音。 |
| AI 职业规划师 `/ai-chat` | 部分实现 | 多会话、流式 DeepSeek 回复、Markdown/表格渲染、附件文本提取、Web Speech/Whisper 浏览器转写、本地与云端会话恢复；移动端使用可开合会话抽屉。 | 附件内容不进入云端会话；大模型转写下载重；非文本附件能力有边界。 |
...
| 跨页面工作流 | `WorkflowContext` | 内存；登录后写 `user-data` | 含职业报告版本、岗位策略/选择、优化结果、职业计划；生产请求附带 Bearer token。 |
...
| 面试过程与报告 | `InterviewSimulation` / `WorkflowContext` | 进行中状态在内存；最近 10 次报告写 `user-data.interviewReports` | 报告可恢复和下载；进行中的题目位置、计时与未完成回答刷新后仍会丢失。 |
...
- 在发送简历文本、职业报告、岗位上下文或文件至 DeepSeek/文本提取服务前，页面必须显示明确、按次的授权控件；默认不发送。授权文案必须说明数据范围和用途。
- 不得显示、计算或导出缺少可解释评估依据的“匹配度”“综合评分”“录用概率”或类似分数。应以已有证据、缺口、真实 JD 核验重点和下一步行动替代。
...
3. 用户逐题作答；题目仅用于整理职业偏好和下一步验证假设，不向用户展示 trait、计时、分数或人格标签。
4. 测评完成后，用户填写补充信息：专业为必填；简历、价值观和性格为可选。简历上传区保留固定可视区域和滚动预览，价值观与性格输入框限制初始及最大高度，用户可从右下角纵向拉伸。
5. 用户勾选明确授权并点击“完成并生成分析”后，页面只发起一次 DeepSeek 请求，生成 `CareerProfile`：版本号、主方向、最多两个备选方向、带来源的证据、待补齐差距与最多三项 7 天任务。没有简历时仍可生成，并在模型失败时回落到本地基础方案。
6. 结果页只显示职业决策报告，不显示总分、雷达图、人格标签或跨测评趋势。用户可修改主目标岗位、查看最近 10 版报告；报告明确该方向只是待验证假设。
7. 本次报告、最多 12000 字的简历文本、`CareerProfile` 和最多 10 条 `careerReports` 写入 `WorkflowContext` 与登录用户云端状态。进入岗位模块不会自动选择或伪造职位。

**产品事实**：测评题目的选项 trait 和基础规则仍用于底层历史记录，但不得被解释为心理测量分数、职业资格结论或可横向比较的趋势数据。

### 6.2 岗位匹配

1. 用户填写行业、目标岗位、城市、经验阶段、最低薪资和工作方式；不自动请求或生成岗位结果。
2. 用户勾选授权后，才将目标条件、`CareerProfile` 和已保存简历文本交给 DeepSeek。
3. DeepSeek 生成最多三个岗位方向，每项只包含判断理由、已有证据、待补齐项和真实 JD 核验重点；失败时本地根据职业报告生成同结构的方向。
4. 用户选择一个方向后写入 `selectedJob`，作为简历与面试的上下文。
5. 后续简历和面试默认继承这个岗位上下文。

**产品事实**：岗位方向不是招聘网站实时职位池，也不会显示或编造公司、薪资、职位时效、录用概率和匹配百分比。真实 JD 是后续核验的唯一来源。

### 6.3 简历优化

1. 使用已上传简历、测评中简历文本、当前页面上传或直接粘贴文本。
2. 岗位方向只用于确定目标；用户必须另行粘贴真实招聘 JD，系统不会把策略摘要冒充 JD。
3. 用户勾选授权后，才请求 DeepSeek 产出 JSON 简历草稿；每段改写必须提供能在原简历中找到的直接引用，技能必须逐字存在于原文。AI 失败、引用缺失、正文包含占位符或新增原文不存在的数字时，结果被拦截且不保存、不复制、不导出；真实 JD 和职业报告不能作为候选人事实来源。
...
3. 用户勾选授权后，调用 DeepSeek 生成开场、行为、深挖、反问四类问题；失败时使用本地题库。题库完成后才进入面试并启动计时，训练过程中不替换当前题目。
4. 用户输入文本或用浏览器录音转写回答；每个主问题后进入规则生成的追问。
5. 结果记录完成环节、当前信息、待补齐证据和训练优先级，可导出 TXT，并为登录用户保存最近 10 次云端报告。

**不得误解**：

- 摄像头只是本地 `<video>` 预览，不上传视频，不做视频分析；进入页面不会自动申请设备权限。
- 浏览器 TTS 优先选择系统普通话女声，以较稳健的播音主持语速朗读问题；系统无女声时回退到可用中文声音。
- 产品只称“AI 面试官”，不宣称视频数字人能力。
- 结果不会生成总体或分项评分，只显示完成环节、已使用证据、待补齐证据和训练优先级；不能宣传为真实面试能力评估。
...
| AI/文件隐私边界不完整 | 测评、岗位、简历和面试页面已增加按次授权；但尚无隐私政策、删除入口、保留期和后端审计记录。 | 个人数据合规和信任风险。 | 增加隐私政策、删除入口、保留策略与服务端审计。 |
...
| 风险 | 当前事实 | 优先策略 |
| --- | --- | --- |
| 面试结果解释边界 | 已移除由完成度和长度派生的分数；当前反馈仍非逐题语义评估。 | 若要评分，先设计逐题结构化规则、证据引用、用户可见依据和失败降级。 |
| 视频数字人未实现 | 当前已统一称“AI 面试官”，能力为文字提问、女声 TTS 和状态反馈。 | 若要接入视频数字人，先完成实时音视频、成本、隐私和失败方案的 TRD。 |
...
| 工作流数据一致性 | 岗位策略、真实 JD、职业报告、简历草稿和最近 10 次面试报告已写云端；`File` 不能可靠 JSON 化，进行中的面试仍不保存。 | 定义附件对象存储、正式版本模型与进行中任务恢复策略。 |
...
| `npx tsc -p tsconfig.app.json --noEmit` | 通过 | 前端严格 TypeScript 检查通过。 |
...
2026-07-30 基础闭环验证：`npm test` 4/4、`npm run build`、严格 TypeScript 检查均通过。390px 移动聊天无横向溢出且抽屉可操作；认证后返回原测评任务；岗位自由文本可正确带入面试；进入面试不自动申请设备权限；题库生成期间不计时，首题进入后保持稳定且初始进度为 0%；本机浏览器实际选中“婷婷”女声。`npm run lint` 仍存在历史规则错误，未作为本轮准出通过项。
...
2. **再使核心闭环持久化**：附件、简历版本和面试报告必须拥有正式的数据模型、生命周期和版本策略。
3. **再提升结果可信度**：把岗位方向和面试反馈从“文案生成”升级为有证据、有解释、有降级的结构化评估；评分功能仅在评估规则成立后引入。
...
| 术语 | 本项目定义 |
| --- | --- |
| 职业画像 / `CareerProfile` | 可持久化的职业决策数据：主目标方向、备选方向、证据、差距、7 天任务和生成边界；不包含任何声称精确的能力分数。 |
| 目标岗位方向 | `WorkflowContext.selectedJob`，是用户主动选择的待验证方向，也是简历优化和面试训练的关键上下文；不代表真实职位。 |
| 工作流状态 | 在 `WorkflowContext` 中跨页面共享的测评、岗位、真实 JD、简历、职业计划和最近面试报告状态。 |
| 简历草稿 | AI 基于原简历与真实 JD 生成并通过最小事实校验的结构化 `ResumeDraft`；仍必须由用户核对，不是用户原始文件。 |
| AI 面试官 | 文字提问、浏览器普通话女声朗读与状态反馈，不指真实视频数字人。 |
