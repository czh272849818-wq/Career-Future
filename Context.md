...
| 简历优化 `/resume` | 部分实现 | PDF/DOCX 文本提取、粘贴文本、JD 对比、关键词与证据覆盖、AI JSON 草稿、本地降级草稿、可复制正文、中文/英文模板 Word 下载、最近 10 条简历记录。 | 原文件二进制未上云；关键词覆盖不是 ATS 或录用概率；模板替换不能保证复杂版式的语义正确性；上传仅支持 PDF/DOCX 入口。 |
| AI 面试 `/interview` | 部分实现 | 行业/岗位/职责上下文、四类面试、授权后的 AI 题目生成与本地题库回退、主问题+追问、文字回答、浏览器语音输入、摄像头预览、浏览器 TTS、TXT 报告下载。 | “数字人”是 CSS 状态层，不是视频数字人；结果只记录完成环节、已用证据、待补齐项和训练建议，不是面试能力评分；面试结果未持久化。 |
...
2. 读取选中岗位方向/JD，提取命中与缺失关键词，并识别是否已有量化成果表达；这些是编辑提示而非评分。
3. 用户勾选授权后，才请求 DeepSeek 产出 JSON 简历草稿；失败时生成结构化本地降级草稿。
...
3. 用户勾选授权后，调用 DeepSeek 生成开场、行为、深挖、反问四类问题；失败时使用本地题库。
4. 用户输入文本或用浏览器录音转写回答；每个主问题后进入规则生成的追问。
5. 结果记录完成环节、已使用证据、待补齐证据和训练优先级，并导出 TXT。
...
| 工作流数据一致性 | 岗位策略、选择与职业报告版本已写云端；`File` 不能可靠 JSON 化，面试结果仍不保存。 | 定义附件对象存储、正式版本模型与面试历史策略。 |
